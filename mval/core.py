...
__all__ = ['validate_param', 'validate_and_load', 'ParameterNotDefinedError', 'documented_by']
...
# Cell
def validate_and_load(params, defs):
    """
    Check a set of parameters stored in the `param` dictionary to see if they meet the necessary requirements of
    `definitions`.  If they do, they are loaded and returned in the order supplied within dictionary,
    otherwise raise an exception.

    **Parameters**
      - `param`: *Dict[str, Any]*: Keys of params must be present in `mtemplate.definitions.PARAMS` and identify which
                                   parameter it is, and its value is the value we are validating and loading.
      - `defs`: *Dict[str,Tuple[type, function, function]]*: contains the type, restrictions function, and loading function
                                                             for each parameter
    **Returns**
      - *tuple*: containing the validated and loaded parameters (in the same order as the params dictionary).
    """
    for p_name in params:
        p = param[p_name]  # parameter
        try:
            p_type, p_restrictions, p_loading = defs[p_name]  # get the required type, restrictions, and loading function
        except KeyError as error:
            raise ParameterNotDefinedError(p_name)
        p = validate_param(p, p_type, p_name, p_restrictions)  # check if the parameter meets the type and restrictions
        v = p_loading(p)  # load the parameter using the specified function
        param[p_name] = v  # assign the loaded value to the parameter entry

    return tuple([param[i] for i in param])  # ensure that this is in the same order as supplied


# Cell
class ParameterNotDefinedError(Exception):
    """
    This exception is raised when a p_name is not a key in the `defs` dictionary.
    """
    def __init__(self, value=None):
        base_msg = ("Either add parameter details to `defs` dictionary or choose from one of the " +
                    "following defined parameters: \n\n\t" + ", ".join(['"{}"'.format(i) for i in PARAMS]))
        self.message = "" + base_msg if value is None else 'Unrecognized parameter name: "{}".  '.format(value) + base_msg
        super(ParamNotDefinedError, self).__init__(self.message)

# Cell
def documented_by(original_doc_func):
    """
    This decorator function allows functions to copy docstrings from other functions.  And appends any docstring it has to the
    end.
    ...
    **Examples**
    ...
    def wrapper(copied_doc_func):
        copied_doc_func.__doc__ = original_doc_func.__doc__ + copied_doc_func.__doc__
